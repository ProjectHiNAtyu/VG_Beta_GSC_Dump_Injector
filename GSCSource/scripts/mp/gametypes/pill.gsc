// S4 GSC SOURCE
// Generated by https://github.com/xensik/gsc-tool

_id_0271()
{
    _id_07B3::_id_021D();
    _id_07B3::_id_D849();
    var_0[0] = _id_0A69::_id_6A43();
    var_0[1] = "grind";
    var_0[2] = "dom";
    _id_07AD::_id_0271( var_0 );

    if ( isusingmatchrulesdata() )
    {
        level._id_81CB = ::_id_81CB;
        [[ level._id_81CB ]]();
        level thread _id_0A69::_id_C2DD();
    }
    else
    {
        _id_0A69::_id_C29C( _id_0A69::_id_6A43(), 0, 0, 9 );
        _id_0A69::registersharedfunc( _id_0A69::_id_6A43(), 600 );
        _id_0A69::_id_C29F( _id_0A69::_id_6A43(), 85 );
        _id_0A69::_id_C29B( _id_0A69::_id_6A43(), 1 );
        _id_0A69::_id_C2C2( _id_0A69::_id_6A43(), 1 );
        _id_0A69::_id_C270( _id_0A69::_id_6A43(), 0 );
        _id_0A69::_id_C24F( _id_0A69::_id_6A43(), 0 );
    }

    _id_FC7A();
    level._id_4CCA = [];
    scripts\mp\gametypes\obj_grindzone::_id_021D();
    level._id_EF62 = 1;
    level._id_AB9A = ::_id_AB9A;
    level._id_AB46 = ::_id_AB46;
    level._id_6D2C = ::_id_6D2C;
    level._id_AB2C = ::_id_AB2C;
    level._id_9FD5 = ::_id_AB91;
    level._id_3B4E["vanish"] = _func_0139( "vfx/core/impacts/small_snowhit" );
    _id_07A9::_id_021D();
    level._id_7C59 = "icon_waypoint_generic";
}

_id_81CB()
{
    _id_0A69::_id_D4D3();
    _func_01D2( "scr_pill_bankTime", getmatchrulesdata( "pillData", "bankTime" ) );
    _func_01D2( "scr_pill_bankRate", getmatchrulesdata( "pillData", "bankRate" ) );
    _func_01D2( "scr_pill_bankCaptureTime", getmatchrulesdata( "pillData", "bankCaptureTime" ) );
    _func_01D2( "scr_pill_megaBankLimit", getmatchrulesdata( "pillData", "megaBankLimit" ) );
    _func_01D2( "scr_pill_bankBonus", getmatchrulesdata( "pillData", "megaBankBonus" ) );
    _func_01D2( "scr_pill_halftime", 0 );
    _id_0A69::_id_C24F( "grind", 0 );
    _func_01D2( "scr_pill_promode", 0 );
}

_id_AB9A()
{
    _func_01CE( "auto_change" );

    if ( !isdefined( game["switchedsides"] ) )
        game["switchedsides"] = 0;

    _id_0A69::_id_D610( "allies", &"OBJECTIVES/WAR" );
    _id_0A69::_id_D610( "axis", &"OBJECTIVES/WAR" );

    if ( level._id_E427 )
    {
        _id_0A69::_id_D60E( "allies", &"OBJECTIVES/WAR" );
        _id_0A69::_id_D60E( "axis", &"OBJECTIVES/WAR" );
    }
    else
    {
        _id_0A69::_id_D60E( "allies", &"OBJECTIVES/WAR_SCORE" );
        _id_0A69::_id_D60E( "axis", &"OBJECTIVES/WAR_SCORE" );
    }

    _id_0A69::_id_D609( "allies", &"OBJECTIVES/WAR_HINT" );
    _id_0A69::_id_D609( "axis", &"OBJECTIVES/WAR_HINT" );
    _id_827A();
    _id_4039();
    level._id_4CC1 = ::_id_4CC1;
    scripts\mp\gametypes\br_pickups::_id_8126();
    scripts\mp\gametypes\br_pickups::_id_8240();
    _id_E31C();
    level thread _id_C520();
    level thread _id_21B3();
}

_id_FC7A()
{
    scripts\mp\gametypes\common::_id_FC36();
    level._id_21B4 = _id_0A66::_id_507A( "bankTime", 2, 0, 10 );
    level._id_21B1 = _id_0A66::_id_507C( "bankRate", 1, 1, 1000 );
    level._id_21A5 = _id_0A66::_id_507C( "bankCaptureTime", 0, 0, 10 );
    level._id_9D21 = _id_0A66::_id_507C( "megaBankLimit", 5, 5, 15 );
    level._id_9D1F = _id_0A66::_id_507C( "megaBankBonus", 150, 0, 750 );
}

_id_AB91()
{
    if ( isdefined( self._id_ED71 ) )
        self _meth_82F6( "ui_pillage_currency", self._id_ED71 );
}

_id_4039()
{
    level._id_4CC7 = [];

    for ( var_0 = 0; var_0 < 30; var_0++ )
    {
        var_1[0] = _func_0205( "script_model", ( 0, 0, 0 ) );
        var_1[0] setmode( "military_dogtags_iw8_orange" );
        var_1[1] = _func_0205( "script_model", ( 0, 0, 0 ) );
        var_1[1] setmode( "military_dogtags_iw8_blue" );
        var_1[0] _meth_82CA( "mp_dogtag_spin" );
        var_1[1] _meth_82CA( "mp_dogtag_spin" );
        var_1[0] hide();
        var_1[1] hide();
        var_1[0] _meth_85A2();
        var_1[1] _meth_85A2();
        var_2 = _func_0205( "trigger_radius", ( 0, 0, 0 ), 0, 32, 32 );
        var_2._id_045A = "trigger_dogtag";
        var_2 hide();
        var_3 = _func_020F();
        var_3._id_048F = "useObject";
        var_3._id_411F = var_2._id_02EA;
        var_3._id_54F3 = var_2 getentitynumber();
        var_3._id_90A1 = 0;
        var_3._id_1042E = var_1;
        var_3._id_A96B = ( 0, 0, 16 );
        var_3._id_047A = var_2;
        var_3._id_F634 = "proximity";
        var_3 _id_07AD::_id_15B7( "none" );
        level._id_4CC7[level._id_4CC7.size] = var_3;
    }
}

_id_6D93()
{
    var_0 = level._id_4CC7[0];
    var_1 = gettime();

    foreach ( var_3 in level._id_4CC7 )
    {
        if ( !isdefined( var_3._id_90A1 ) )
            continue;

        if ( var_3._id_839C == "none" )
        {
            var_0 = var_3;
            break;
        }

        if ( var_3._id_90A1 < var_1 )
        {
            var_1 = var_3._id_90A1;
            var_0 = var_3;
        }
    }

    var_0 notify( "reset" );
    var_0 _id_07AD::_id_81D0();
    var_0._id_90A1 = gettime();
    return var_0;
}

_id_E388( var_0, var_1 )
{
    var_2 = var_0 + ( 0, 0, 14 );
    var_3 = ( 0, _func_01B6( 360 ), 0 );
    var_4 = anglestoforward( var_3 );
    var_5 = _func_01B7( 30, 150 );
    var_6 = var_2 + var_5 * var_4;
    var_2 = physicsexplosionsphere( var_2, var_6 );
    var_7 = _id_6D93();
    var_7._id_411F = var_2;
    var_7._id_047A._id_02EA = var_2;
    var_7._id_1042E[0]._id_02EA = var_2;
    var_7._id_1042E[1]._id_02EA = var_2;
    var_7._id_047A _meth_83C3();
    var_7 _id_07AD::_id_15B7( "any" );
    var_7._id_1042E[0] thread _id_DCF4( var_7, _id_0A69::_id_6BC3( var_1 )[0] );
    var_7._id_1042E[1] thread _id_DCF4( var_7, var_1 );
    var_7._id_1042E[0] _meth_85A2();
    var_7._id_1042E[1] _meth_85A2();
    _func_019D( var_2, "mp_grind_token_drop" );
    return var_7;
}

_id_DCF4( var_0, var_1 )
{
    var_0 endon( "death" );
    var_0 endon( "reset" );
    self hide();

    foreach ( var_3 in level._id_B758 )
    {
        if ( _id_B660( var_3 ) )
        {
            if ( var_3._id_045B == var_1 )
                self _meth_83CA( var_3 );

            if ( var_3._id_045B == "spectator" && var_1 == "allies" )
                self _meth_83CA( var_3 );
        }
    }

    for (;;)
    {
        level waittill( "joined_team" );
        self hide();

        foreach ( var_3 in level._id_B758 )
        {
            if ( _id_B660( var_3 ) )
            {
                if ( var_3._id_045B == var_1 )
                    self _meth_83CA( var_3 );
            }

            if ( var_3._id_045B == "spectator" && var_1 == "allies" )
                self _meth_83CA( var_3 );

            if ( var_0._id_103CA == var_3._id_045B && var_3 == var_0._id_006E )
                _id_07D0::_id_FB9B( var_0._id_A8F3, "invisible" );
        }
    }
}

_id_B660( var_0 )
{
    return 1;
}

_id_A135( var_0 )
{
    level endon( "game_ended" );
    var_0 endon( "deleted" );
    var_0 endon( "reset" );

    for (;;)
    {
        var_0._id_047A waittill( "trigger", var_1 );

        if ( !_id_0A74::_id_89D3( var_1 ) )
            continue;

        if ( var_1 _id_0A74::_id_8AB5() || isdefined( var_1._id_E304 ) )
            continue;

        if ( isdefined( var_1._id_00DE ) && var_1._id_00DE == "script_vehicle" )
            continue;

        if ( _func_0102( var_1 ) && isdefined( var_1._id_02F2 ) )
            var_1 = var_1._id_02F2;

        var_0._id_1042E[0] hide();
        var_0._id_1042E[1] hide();
        var_0._id_047A hide();
        var_0._id_411F = ( 0, 0, 1000 );
        var_0._id_047A._id_02EA = ( 0, 0, 1000 );
        var_0._id_1042E[0]._id_02EA = ( 0, 0, 1000 );
        var_0._id_1042E[1]._id_02EA = ( 0, 0, 1000 );
        var_0 _id_07AD::_id_15B7( "none" );

        if ( var_0._id_045B != var_1._id_045B )
        {
            var_1 _id_B792( var_1._id_ED71 + 1 );
            var_1 thread _id_0A76::_id_6FE6( "tag_collected" );
        }

        var_1 _meth_827B( "mp_killconfirm_tags_pickup" );

        if ( isdefined( level._id_EB96 ) && level._id_EB96 )
        {
            if ( isdefined( var_1._id_3E37 ) && var_1._id_3E37 )
                var_1 _id_0798::_id_D4E1( "kill" );
            else
                var_1 _id_0798::_id_AA7E( undefined, var_1 );
        }

        _func_019D( var_1._id_02EA, "mp_grind_token_pickup" );
        break;
    }
}

_id_AB46( var_0 )
{
    var_0._id_89FD = 0;
    var_0 thread _id_A109();
}

_id_B792( var_0 )
{
    self._id_ED71 = var_0;
    self._id_01DB = var_0;
    self _meth_82F6( "ui_pillage_currency", var_0 );
}

_id_A109()
{
    self endon( "disconnect" );

    for (;;)
    {
        _id_077B::_id_1087E( "joined_team", "joined_spectators" );
        _id_B792( 0 );
    }
}

_id_78E7( var_0 )
{
    level waittill( "game_ended" );

    if ( isdefined( var_0 ) )
        var_0._id_0047 = 0;
}

_id_4051()
{
    var_0 = getentarray( "grind_location", "targetname" );

    foreach ( var_2 in var_0 )
    {
        var_3 = scripts\mp\gametypes\obj_grindzone::_id_D88B( var_2 );
        var_3 thread _id_CA71();
        level._id_A8D5[var_3._id_A8CE] = var_3;
    }
}

_id_88BF( var_0, var_1 )
{
    if ( _id_0A74::_id_89D3( var_0 ) && var_0 _meth_81EF( var_1._id_047A ) && var_1._id_AD9B == var_0._id_045B )
        return 1;

    return 0;
}

_id_CA71()
{
    level endon( "game_ended" );

    for (;;)
    {
        self._id_047A waittill( "trigger", var_0 );

        if ( self._id_E578 )
            continue;

        if ( _func_0102( var_0 ) )
            continue;

        if ( !_func_0117( var_0 ) )
            continue;

        if ( var_0._id_89FD )
            continue;

        var_0._id_89FD = 1;
        level thread _id_BD6E( var_0, self );
    }
}

_id_C520()
{
    level waittill( "game_ended" );

    foreach ( var_1 in level._id_B758 )
    {
        if ( !isdefined( var_1 ) )
            continue;

        if ( !isdefined( var_1._id_ED71 ) )
            continue;

        var_1._id_ED71 = 0;
    }
}

_id_BD6E( var_0, var_1 )
{
    while ( var_0._id_ED71 && _id_88BF( var_0, var_1 ) && !var_1._id_E578 )
    {
        var_0 _meth_827E( "mp_grind_token_banked", var_0 );
        var_2 = level._id_21B1;

        if ( var_2 > var_0._id_ED71 )
            var_2 = var_0._id_ED71;

        _id_CBE7( var_0, var_2 );

        for ( var_3 = 0; var_3 < var_2; var_3++ )
            var_0 thread _id_0A76::_id_6FE6( "tag_score" );

        if ( isdefined( level._id_EB96 ) && level._id_EB96 && isdefined( var_0._id_3E37 ) && var_0._id_3E37 )
            var_0 _id_0798::_id_D4E1( "kill" );

        wait( level._id_21B4 );
    }

    var_1 scripts\mp\gametypes\obj_grindzone::_id_D5FC();
    var_0._id_89FD = 0;
}

_id_CBE7( var_0, var_1 )
{
    var_0 _id_B792( var_0._id_ED71 - var_1 );
    _id_07AE::_id_6FE4( var_0._id_045B, var_1, 0 );
}

_id_827A()
{
    _id_07EE::_id_D454( "BigTDM", "Crit_Frontline" );
    level._id_E324 = ( 0, 0, 0 );
    level._id_E321 = ( 0, 0, 0 );
    _id_07EE::_id_1111( "mp_tdm_spawn_allies_start" );
    _id_07EE::_id_1111( "mp_tdm_spawn_axis_start" );
    _id_07EE::_id_110D( "allies", "mp_tdm_spawn_allies_start" );
    _id_07EE::_id_110D( "axis", "mp_tdm_spawn_axis_start" );
    _id_07EE::_id_C2AB( "normal_allies", "mp_tdm_spawn_allies_start" );
    _id_07EE::_id_C2AB( "normal_axis", "mp_tdm_spawn_axis_start" );
    _id_07EE::_id_C2AB( "fallback", "mp_tdm_spawn_secondary" );
    level._id_9A95 = _id_07EE::_id_5AC1( level._id_E324, level._id_E321 );
    setgameendtime( level._id_9A95 );
    _id_07EE::_id_0EBC();
}

_id_6D2C()
{
    var_0 = self._id_0309["team"];

    if ( game["switchedsides"] )
        var_0 = _id_0A69::_id_6BC3( var_0 )[0];

    if ( _id_07EE::_id_DC03() )
    {
        var_1 = _id_07EE::_id_6D30( "mp_tdm_spawn_" + var_0 + "_start" );
        var_2 = _id_07EE::_id_6D2F( var_1 );
    }
    else if ( var_0 == "allies" )
        var_2 = _id_07EE::_id_6D2C( self, var_0, "normal_allies", "fallback" );
    else
        var_2 = _id_07EE::_id_6D2C( self, var_0, "normal_axis", "fallback" );

    return var_2;
}

_id_AB2C( var_0, var_1, var_2, var_3, var_4, var_5 )
{
    if ( istrue( level._id_5CB3 ) )
        _id_07A9::_id_5CA9( var_1, var_0, gettime(), "kill_by_" + var_1._id_045B );

    var_0 scripts\mp\gametypes\plunder::_id_B792( var_0._id_ED71 + 1000 );
    scripts\mp\gametypes\common::_id_AA76( var_0, var_1, var_2, var_3, var_4, var_5 );
}

_id_5047( var_0, var_1 )
{
    if ( _func_0102( var_0 ) )
        return;

    if ( var_0._id_ED71 > 9 )
        var_2 = 10;
    else if ( var_0._id_ED71 > 0 )
        var_2 = var_0._id_ED71;
    else
        var_2 = 0;

    for ( var_3 = 0; var_3 < var_2; var_3++ )
    {
        var_4 = _id_E388( var_0._id_02EA, var_0._id_045B );
        var_4._id_045B = var_0._id_045B;
        var_4._id_103C0 = var_0;
        var_4._id_006E = var_1;
        level notify( "new_tag_spawned", var_4 );
        level thread _id_A135( var_4 );
    }

    var_5 = var_0._id_ED71 - var_2;
    var_5 = int( _func_0147( 0, var_5 ) );
    var_0 _id_B792( var_5 );
}

_id_4CC1( var_0 )
{

}

_id_C4FF()
{
    self endon( "game_ended" );

    for (;;)
    {
        if ( getdvar( "scr_devRemoveDomFlag", "" ) != "" )
        {
            var_0 = getdvar( "scr_devRemoveDomFlag", "" );

            foreach ( var_2 in level._id_A8D5 )
            {
                if ( isdefined( var_2._id_A8CE ) && var_2._id_A8CE == var_0 )
                {
                    var_2 notify( "stop_trigger" + var_2._id_A8CE );
                    var_2 _id_07AD::_id_15B7( "none" );
                    var_2._id_047A = undefined;
                    var_2 notify( "deleted" );
                    var_2._id_10418 = "none";
                    var_2 _id_07AD::_id_D60F( undefined, undefined );
                    var_3 = [];

                    foreach ( var_5 in level._id_A8D5 )
                    {
                        if ( var_5._id_A8CE != var_0 )
                            var_3[var_5._id_A8CE] = var_5;
                    }

                    level._id_A8D5 = var_3;
                    break;
                }
            }

            _func_01D2( "scr_devRemoveDomFlag", "" );
        }

        wait 1;
    }
}

_id_B167()
{
    self endon( "game_ended" );

    for (;;)
    {
        if ( getdvar( "scr_devPlaceDomFlag", "" ) != "" )
        {
            var_0 = getdvar( "scr_devPlaceDomFlag", "" );
            var_1 = undefined;
            var_2 = getentarray( "grind_location", "targetname" );

            foreach ( var_4 in var_2 )
            {
                if ( "_" + var_4._id_CD8C == var_0 )
                    var_1 = var_4;
            }

            var_1._id_02EA = level._id_B758[0]._id_02EA;
            var_1._id_AD9B = "neutral";
            var_6 = var_1._id_02EA + ( 0, 0, 32 );
            var_7 = var_1._id_02EA + ( 0, 0, -32 );
            var_8 = _id_077A::_id_C042( var_6, var_7, undefined, _id_077A::_id_3ED4( 1 ) );
            var_1._id_02EA = var_8["position"];
            var_1._id_FB2C = _func_025B( var_8["normal"] );
            var_1._id_01C1 = anglestoforward( var_1._id_FB2C );
            var_1._id_034F = anglestoright( var_1._id_FB2C );
            var_9[0] = _func_0205( "script_model", var_1._id_02EA );
            var_9[0]._id_0054 = var_1._id_0054;
            var_10 = _func_0205( "trigger_radius", var_1._id_02EA, 0, 90, 128 );
            var_10._id_CD8C = var_1._id_CD8C;
            var_1 = var_10;
            var_11 = _id_07AD::_id_404C( "neutral", var_1, var_9, ( 0, 0, 100 ) );

            if ( isdefined( var_1._id_A8CE ) )
                var_11._id_A8CE = var_1._id_A8CE;
            else
                var_11._id_A8CE = var_11 _id_07AD::_id_6AD2();

            if ( isdefined( var_1._id_7C58 ) )
                var_11._id_7C58 = var_1._id_7C58;
            else
                var_11._id_7C58 = var_11 _id_07AD::_id_6AD2();

            var_11 thread _id_CA71();
            var_11 _id_07AD::_id_15B7( "enemy" );
            var_11 _id_07AD::_id_D8C7( level._id_21A5 );
            var_11 _id_07AD::_id_D60F( "waypoint_defend", "waypoint_captureneutral" );
            var_11 _id_07AD::_id_D8D5( "any" );
            var_11 _id_07AD::_id_3144( 1 );
            var_11._id_ABB8 = scripts\mp\gametypes\obj_grindzone::_id_110D6;
            var_11._id_AA66 = scripts\mp\gametypes\obj_grindzone::_id_110D7;
            var_11._id_ABB5 = scripts\mp\gametypes\obj_grindzone::_id_110D4;
            var_11._id_AA7A = scripts\mp\gametypes\obj_grindzone::_id_110D0;
            var_11._id_ABB4 = scripts\mp\gametypes\obj_grindzone::_id_110D3;
            var_11._id_37CF = level._id_21A5 * 1000;
            var_6 = var_11._id_1042E[0]._id_02EA + ( 0, 0, 32 );
            var_7 = var_11._id_1042E[0]._id_02EA + ( 0, 0, -32 );
            var_12 = _id_077A::_id_3EC3( 1, 1, 1, 1, 0, 1, 1 );
            var_13 = [];
            var_8 = _id_077A::_id_C042( var_6, var_7, var_13, var_12 );
            var_11._id_228F = var_8["position"];
            var_14 = _func_025B( var_8["normal"] );
            var_14 = -1 * var_14;
            var_11._id_228E = anglestoforward( var_14 );
            var_11 scripts\mp\gametypes\obj_grindzone::_id_D5FB();
            level._id_A8D5[var_11._id_A8CE] = var_11;
            _func_01D2( "scr_devPlaceDomFlag", "" );
        }

        wait 1;
    }
}

_id_E31C()
{
    _id_0980::_id_8258();
    var_0 = _id_077B::_id_6D7C( "pillage_cache_spawn", "targetname" );
    var_1 = int( 100 * var_0.size / 100 );
    var_0 = _id_077B::_id_1B94( var_0 );
    level._id_412C = [];

    for ( var_2 = 0; var_2 < var_1; var_2++ )
        _id_E31B( var_0[var_2] );
}

_id_E31B( var_0 )
{
    var_1 = scripts\mp\gametypes\br_pickups::_id_E333( "Pillage_Cache", var_0._id_02EA + level._id_2A6F._id_2A14[0], ( 0, 0, 0 ) );
}

_id_6FE0()
{
    scripts\mp\gametypes\plunder::_id_B792( self._id_ED71 + 1000 );
}

_id_21B3()
{
    self endon( "game_ended" );
    var_0 = getentarray( "grind_location", "targetname" );
    wait 5;
    var_1 = [];
    var_2 = [];

    for (;;)
    {
        var_0 = _id_077B::_id_1B94( var_0 );
        var_3 = 0;
        var_4 = 0;

        while ( var_3 < 1 )
        {
            foreach ( var_4, var_6 in var_0 )
            {
                if ( var_3 == 1 )
                    break;

                if ( ( !isdefined( var_6._id_84D5 ) || !var_6._id_84D5 ) && ( !isdefined( var_6._id_FE4C ) || !var_6._id_FE4C ) )
                {
                    var_7 = scripts\mp\gametypes\obj_grindzone::_id_D88B( var_0[var_4] );
                    var_7 _id_07AD::_id_15B7( "any" );
                    var_7 _id_07AD::_id_D8D5( "any" );
                    _id_07D0::_id_A89E( var_7._id_A8F5, 0 );
                    var_7._id_96F3 = 0;
                    var_7 _id_07AD::_id_D60F( "icon_waypoint_generic" );
                    var_7._id_96F3 = 1;
                    _func_0162( var_7._id_A8F5, &"MP_INGAME_ONLY/PILLAGE_DEPOT_OPEN" );
                    _func_0165( var_7._id_A8F5, &"MP_INGAME_ONLY/PILLAGE_DEPOT_OPEN" );
                    _func_0163( var_7._id_A8F5, &"MP_INGAME_ONLY/PILLAGE_DEPOT_OPEN" );
                    _func_0164( var_7._id_A8F5, &"MP_INGAME_ONLY/PILLAGE_DEPOT_OPEN" );
                    var_7 thread _id_CA71();
                    var_1[var_4] = var_7;
                    var_7._id_E06E = var_6;
                    var_6._id_84D5 = 1;
                    var_3++;
                }
            }

            if ( var_3 < 1 )
            {
                foreach ( var_6 in var_0 )
                    var_6._id_FE4C = 0;
            }
        }

        wait 5;

        foreach ( var_6 in var_1 )
        {
            var_6 _id_07AD::_id_15B7( "none" );
            var_6 _id_07AD::_id_D8D5( "none" );
            var_6 _id_07AD::_id_C2F2();
            var_6._id_10418 = "none";
            var_6._id_0383 _meth_809A();
            var_6._id_E06E._id_FE4C = 1;
            var_6._id_E06E._id_84D5 = 0;
        }

        var_1 = [];
        wait 5;
    }
}
